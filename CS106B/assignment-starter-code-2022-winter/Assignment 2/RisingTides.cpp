                            ...
                            double height)
{
    ...
    Vector<GridLocation> dirs = {
        GridLocation{-1, 0}, // Up
        GridLocation{1, 0},  // Down
        GridLocation{0, -1}, // Left
        GridLocation{0, 1}   // Right
    };

    // init queue
    for (const GridLocation &source : sources)
    {
        if (terrain.inBounds(source) && terrain[source.row][source.col] <= height)
        {
    ...
    while (!queue.isEmpty())
    {
        GridLocation curr_loc = queue.dequeue();

        for (const GridLocation &dir : dirs)
        {
            GridLocation next_loc = {curr_loc.row + dir.row, curr_loc.col + dir.col};

            if (terrain.inBounds(next_loc) && !flooded[next_loc.row][next_loc.col] && terrain[next_loc.row][next_loc.col] <= height)
            {
                flooded[next_loc.row][next_loc.col] = true;
                queue.enqueue(next_loc);
            }
        }
    }
